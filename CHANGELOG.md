...
- Added "Forgot password?" (`/forgot-password` → emailed link → `/reset-password`) — the admin-created-account model had no recovery path if someone forgot their password and wasn't near their admin. Uses Supabase's built-in auth email service; requires the redirect URL allowlisted per-environment (documented in README).
