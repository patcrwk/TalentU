...
- Password recovery does use email, unlike account creation: `/forgot-password` → `resetPasswordForEmail()` → Supabase's default auth email service → `/reset-password`. This only works if the redirect URL is allowlisted per-environment in Supabase's dashboard (Authentication > URL Configuration) — see README step 6. `/account` covers the simpler in-session "change my password" case with no email involved.
  - **Gotcha:** Supabase's default reset-password email template links straight to its own `/auth/v1/verify` endpoint, which consumes the one-time token on the very first HTTP fetch — including a mail provider's own link-prescanning (Gmail does this too, not just corporate gateways), which burns the token before the person ever clicks it, surfacing as `otp_expired` even on a link that's seconds old. Fix: in Supabase Dashboard > Authentication > Email Templates > Reset Password, change the link to `{{ .SiteURL }}/reset-password?token_hash={{ .TokenHash }}&type=recovery` instead of `{{ .ConfirmationURL }}`. `ResetPasswordForm.tsx` reads `token_hash`/`type` from the query string and calls `verifyOtp()` itself, so verification only happens when a real browser actually runs the page's JS — a bot's plain GET of the link no longer consumes anything. The old hash-fragment flow (`PASSWORD_RECOVERY` event) is kept as a fallback in the same component in case the template hasn't been switched over yet.
